...
module.exports = {
	...
	stable: true,
	args: true,
	usage: '[boolean? list] <channel ID / name or direct tag>',
	onTrigger: async (message, args) => {
		...
		if (args.length)
		{
			if (!message.member.permissions.has('MANAGE_CHANNELS')) return message.channel.send(`**${message.member.displayName}**, you are lacking permission to do so.`);
			else
			{
				switch (args[0])
				{
